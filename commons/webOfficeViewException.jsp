...
<script type="text/javascript">
	$(document).ready(function(){
		...
		if(type=='txt'){
			$.post("${ctx}/contentSelectAction.do?method=download&filename=${param.filename}&fileaddress=${param.fileaddress}",function(msg){
...
<body >
	<div id="officecontrol"  style="height:950px"> 
	 <c:if test="${fn:contains('jpg,jpeg,gif,png',fn:toLowerCase(param.fileType))}">
	 	<img src="${ctx}/contentSelectAction.do?method=download&filename=${param.filename}&fileaddress=${param.fileaddress}"/>
	 </c:if>
                ...
                <div id=statusBar style="height:20px;width:100%;background-color:#c0c0c0;font-size:12px;"></div>
								...
								function init(){
									var TANGER_OCX_OBJ='';
						       	    ...
						       	    TANGER_OCX_OBJ.BeginOpenFromURL("${ctx}/contentSelectAction.do?method=download&filename=${param.filename}&fileaddress=${param.fileaddress}");
								}
								</script>
								</c:if>
