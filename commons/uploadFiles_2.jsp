...
function upload_getFileName(controlID,fileBox) {
	if (fileBox.value){
		 ...
		 var fileType = path.substring(path.lastIndexOf('.'),path.lenth);
	...
	<table class="listTable"  id="annexTable" >
		...
		<c:forEach items="${pubannexLst}" var="annex" varStatus="index">
			<tr id="historytr${annex.id}" 
				ondblclick="window.document.location.href='pubAnnexAction.do?method=download&id=${annex.id}'">
				<td>
					${index.count }
				</td>
				<td 
