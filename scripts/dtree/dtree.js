...
function dTree(objName) {
	this.config = {target:null, folderLinks:true, useSelection:true, useCookies:true, useLines:true, useIcons:true, useStatusText:false, closeSameLevel:false, inOrder:false, useCheckbox:false,useRadio:false};
	...
	this.aNodes = [];
	this.aIndent = [];
	this.root = new Node(-99);
	this.selectedNode = null;
	this.selectedFound = false;
	this.completed = false;
	this.checkboxName = "";
	this.fnNodeCallback=null;
}
dTree.prototype.setIconPath = function (path) {
	for (var i in this.icon) {
		this.icon[i] = path + this.icon[i];
	}
};
...
//Select Sub nodes
dTree.prototype.selectParentNode=function(nId) {
	...
	var checked = document.getElementById(this.checkboxName+"_"+cn.id).checked;

	if(checked) {
		for (n = 0; n < this.aNodes.length; n++) {
			var parent = this.aNodes[n];
			if (parent.id == cn.pid) {
				var ccn = document.getElementById(this.checkboxName+"_"+kid.id);
				if(ccn) {
					ccn.checked=checked;
					this.selectParentNode(kid.id);
				}
				break;
			}
		}
...
//Select kid nodes
dTree.prototype.selectKidNode=function(nId) {
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].id == nId) {
			nId = n;
			break;
	...
	for (n = 0; n < this.aNodes.length; n++) {
		var kid = this.aNodes[n];
		if (kid.pid == cn.id) {
			var ccn = document.getElementById(this.checkboxName+"_"+kid.id);
			if(ccn) {
				ccn.checked=checked;
				this.selectKidNode(kid.id);
			}
		}
	}
};
...
dTree.prototype.node = function (node, nodeId) {
	...
	if(this.config.useCheckbox) {
		str += "<input style=\"width:16px;height:16px;\" type=\"checkbox\" id=\""+this.checkboxName+"_"+node.id+"\" name=\""+this.checkboxName+"\" value=\""+node.id+"\"";
		if(this.fnNodeCallback) {
			str += " onclick=\""+this.obj+".fnNodeCallback('"+node.id+"')\"";
		}
		if(node.checked) {
			str += " checked=\"checked\"";
		}
	...
	if(this.config.useRadio) {
		str += "<input style=\"width:16px;height:16px;\" type=\"radio\" id=\""+this.checkboxName+"_"+node.id+"\" name=\""+this.checkboxName+"\" value=\""+node.id+"\"";
		if(this.fnNodeCallback) {
			str += " onclick=\""+this.obj+".fnNodeCallback('"+node.id+"')\"";
		}
		str +=">";
...
dTree.prototype.setChecked = function(nId, checked) {
	for (var n = 0; n < this.aNodes.length; n++) {
		if (this.aNodes[n].id == nId) {
			nId = n;
			break;
