...
function CLASS_CHECK(){
	...
	var aUsage = {
		...
		"num+":"^([+]?)\\d*\\.?\\d+$",									//正数
		"num-":"^-\\d*\\.?\\d+$",										//负数
		"float":"^([+-]?)\\d*\\.\\d+$",									//浮点数，限制输入2位小数
		"float4":"^([+-]?)\\d*\\.\\d+$",									//浮点数,限制输入4位小数
		"float+":"^([+]?)\\d*\\.\\d+$",									//正浮点数
		"float-":"^-\\d*\\.\\d+$",										//负浮点数
																		//邮件
					...
					switch(sUsage.toLowerCase ()){
						...
						case "float":
							els[i].onkeypress	= function(e){
								var chr;
								if(e)chr=e.charCode; 
								else chr=window.event.keyCode;
								if(chr==0){
									return true;
								}else {
									var k=chr;
									var value=this.value;
									if(k==45&&value.length==0){
										return true;
									}
									if((k==46)||(k<=57&&k>=48)) {
										if(k==46&&value.length==0) return false; 
										if(k==46&&value.length==1&&value=='-') return false; 
										if(k==46&&value.indexOf('.')>-1) return false; 
							...
							els[i].onpaste		= function(e){
								if(window.clipboardData){
									if(e==null){
										if(!/^\d*\.?\d{0,2}$/.test(clipboardData.getData('text')))return false; 
										return true;
									}else{
										return false;
									}
								}else{
									//alert("请选择IE浏览器进行粘贴操作，其他浏览器不支持此操作！");
								}
								return false;
							}
							els[i].ondragenter	= function(e){return false;}
						...
						case "num":
							...
							els[i].onkeypress	= 
							function(e)
							{var chr;if(e)chr=e.charCode; else chr=window.event.keyCode;
							if(chr==0){return true;}
							 else 
							 {
							 //2010-9-7 增加，限制输入2位小数
							 var docSel = document.selection.createRange();
							 var oSel = docSel.duplicate();
							 oSel.text = "";
                             var srcRange = this.createTextRange();
                             oSel.setEndPoint("StartToStart", srcRange);
                             var str = oSel.text + String.fromCharCode(chr)+ srcRange.text.substr(oSel.text.length);
							 return /^\d*\.?\d{0,2}$/.test(str)||(this.value.indexOf('.')<0?String.fromCharCode(chr)==".":false)||(this.value.indexOf('-')<0?String.fromCharCode(chr)=="-":false);}	//2010-09-07 修改 lt 解决小数点数量未控制的问题,以及限制输入2位小数				
							 }
							els[i].onpaste		= function(e){if(e==null)return !clipboardData.getData('text').match(/[^\d\.\+\-]/); else return false;}
							els[i].ondragenter	= function(e){return false;}
							els[i].style.imeMode= "disabled";
						...
						case "float4":
							els[i].onkeypress	= function(e){
								...
								if(chr==0){
									...
									if((k==46)||(k<=57&&k>=48)) {
										...
										if(k==46&&value.length==1&&value=='-') return false; 
										if(k==46&&value.indexOf('.')>-1) return false; 
										if(!/^\d*\.?\d{0,4}$/.test(value))return false; 
										return true;
									} else{
										return false;
									} 
							...
							els[i].onpaste		= function(e){
								if(window.clipboardData){
									if(e==null){
										if(!/^\d*\.?\d{0,4}$/.test(clipboardData.getData('text')))return false; 
										return true;
									}else{
										return false;
									}
						...
						case "num+":
							//els[i].onkeypress	= function(e){var chr;if(e)chr=e.charCode; else chr=window.event.keyCode;if(chr==0)return true; else return /\d/.test(String.fromCharCode(chr));}	
							els[i].onkeypress	= function(e){var chr;if(e)chr=e.charCode; else chr=window.event.keyCode;if(chr==0)return true; else return /\d/.test(String.fromCharCode(chr))||(this.value.indexOf('.')<0?String.fromCharCode(chr)==".":false);}		//2010-09-07 修改 lt 解决小数点数量未控制的问题				
							els[i].onpaste		= function(e){if(e==null)return !clipboardData.getData('text').match(/[^\d\.\+]/); else return false;}
							els[i].ondragenter	= function(e){return false;}
							els[i].style.imeMode= "disabled";
							break;
						...
						case "num-":
							//els[i].onkeypress	= function(e){var chr;if(e)chr=e.charCode; else chr=window.event.keyCode;if(chr==0)return true; else return /\d/.test(String.fromCharCode(chr));}					
							els[i].onkeypress	= function(e){var chr;if(e)chr=e.charCode; else chr=window.event.keyCode;if(chr==0)return true; else return /\d/.test(String.fromCharCode(chr))||(this.value.indexOf('.')<0?String.fromCharCode(chr)==".":false)||(this.value.indexOf('-')<0?String.fromCharCode(chr)=="-":false);}	//2010-09-07 修改 lt 解决小数点数量未控制的问题	
							els[i].onpaste		= function(e){if(e==null)return !clipboardData.getData('text').match(/[^\d\.\-]/); else return false;}
							els[i].ondragenter	= function(e){return false;}
							els[i].style.imeMode= "disabled";
							break;
