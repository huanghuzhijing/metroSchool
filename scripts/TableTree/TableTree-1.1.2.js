...
function TableTree4J(objectName,tableTree4JDir){
	...
	//add menu node
	TableTree4J.prototype.addMenuNode=function(menuName,id,pid,booleanOpen,order,url,target,hrefTip,hrefStatusText,classStyle,icon,iconOpen,footnode){
		var nod=new Node(new Array(menuName),id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,footnode);
		this.addNode(nod);
	}		
	
	//add grid node
	TableTree4J.prototype.addGirdNode=function(dataList,id,pid,booleanOpen,order,url,target,hrefTip,hrefStatusText,classStyle,icon,iconOpen,footnode){
		var nod=new Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,footnode);
		this.addNode(nod);
	}
	...
	//异步刷新
	TableTree4J.prototype.codeStaticImgByPath_Ex=function(imgpath,nodeId){
	...
	TableTree4J.prototype.codeNodeIcon=function(node){
		...
		if(node.booleanLeaf==true){
			if(node.icon==null||node.icon==""){
				...
				//异步刷新
				if(node.footnode!=""&&node.footnode==0){
		...
		if(this.config.useLine==true){
			if(node.booleanLastNode==true){
				if(node.booleanLeaf==true){
					...
					//异步刷新
					if(node.footnode!=""&&node.footnode==0&&tmpmap.get(node.id)==null){
						imgstr=this.codeStaticImgByPath_Ex(this.icon.plusBottom,node.id);
					}
				}else{
				...
				if(node.booleanLeaf==true){
					...
					//异步刷新
					if(node.footnode!=""&&node.footnode==0&&tmpmap.get(node.id)==null){
						imgstr=this.codeStaticImgByPath_Ex(this.icon.plus,node.id);
					}
				}else{
	...
	//异步刷新  对树节点更新保存
	TableTree4J.prototype.saveTreeNodeByMapId=function(node){
		this.map.update("treNod"+node.id,node);
	}
		...
		//alert(nods.length);
			for(var i=0;i<nods.length;i++){
				...
				if(childNodes.length>0){
					tmpmap.put(nods[i].id,nods[i].id);//初始化加载，很重要，如果有子节点，则下次点击就不和数据库交互
...
//Data Value Object
function Node(dataList,id,pid,booleanOpen,order,url,target,classStyle,icon,iconOpen,hrefTip,hrefStatusText,footnode){
...
function clickNode(nodeid,tree){
	...
	if(clickNode.booleanLeaf==false){
		...
		if(clickNode.booleanOpen==true){
			...
			if(tree.config.useIcon==true){
				clickImgNode.src=clickNode.icon;
			}
		...
		if(clickNode.footnode!=""&&clickNode.footnode==0){
			queryData(nodeid);
		}
	}
	clickNodeData(nodeid);
	tmpmap.put(nodeid,nodeid);
}

//异步	可做传值操作
function clickNodeData(nodeid){
}

function clickNodeBy(nodeid,tree){
	var clickNode = tree.findTreeNodeByMapId(nodeid);
	if(clickNode.booleanLeaf==false){
		var clickNodeImg;
		if(clickNode.booleanRoot==false||(clickNode.booleanRoot==true&&tree.config.rootNodeBtn==true)){
...
function clickNodeByInitOpen(nodeid,tree){
	var clickNode = tree.findTreeNodeByMapId(nodeid);
	if(clickNode.booleanOpen==false){
		clickNodeBy(nodeid,tree);
	}
}


//通过本节点异步查询
function queryData(nodeid){
	clickNodeBy(nodeid,gridTree);
}
