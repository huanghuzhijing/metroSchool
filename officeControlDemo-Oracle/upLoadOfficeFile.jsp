...
public boolean saveFileToDisk()
{
	...
	try
	{
		...
		if(!"".equalsIgnoreCase(attachFileNameDisk)&&attachFileItem!=null)
		{
...
public boolean deleteDiskFile(int deleteFileId)
{
	...
	try
	{
		...
		if(rs.next())
		{
			...
			if(!"".equalsIgnoreCase(officeFileNameDel))
			{
			...
			if(!"".equalsIgnoreCase(attachFileNameDel)&&attachFileItem!=null)
			{
...
<%
	...
	if(fileId==0) 
	{fileId = GetMaxID();}
	if(fileId!=0&&!"".equalsIgnoreCase(fileName)&&officeFileItem!=null)
	{
		if(deleteDiskFile(fileId))//删除数据库中fileid对应的服务器文件
		{
			...
			else
			{
					...
					rs.next();	
					attachFileNameDisk = rs.getString("ATTACHFILENAMEDISK");
					rs.close();
			}
			...
			if(saveFileToDisk())
			{
				...
				System.out.println("sql string:"+mySqlStr);
				}
			}
			else
