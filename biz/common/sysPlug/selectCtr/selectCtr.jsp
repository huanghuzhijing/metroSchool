...
<body>
	<table class="queryTable" >
		...
		<tr>
			<td>
				<html:form action="contentSelectAction.do?method=selectCtr&paramWindow=${param.paramWindow }&queryCtrType=${param.queryCtrType}&prj_id=${param.prj_id}">
	...
	<ec:table items="contractInfos" var="contractInfo"
		action="contentSelectAction.do?method=selectCtr&queryCtrType=${param.queryCtrType}&prj_id=${param.prj_id}" title="" locale="zh_CN"
		...
		<ec:row highlightRow="true" >
			...
			<ec:column property="null" title="选择" >
				<input type="button"  class="selectButton" icon="icon_confirm" value="${texts['label.select']}"  
				       paramId="${contractInfo.id}" paramName="${contractInfo.secondPartyId.corpname}" paramArray='${contractInfo.ctrCode}@@${contractInfo.ctrName}@@${contractInfo.ctrSort}@@${contractInfo.payClause}@@${contractInfo.prjId.id}@@${contractInfo.prjId.prjCode}@@${contractInfo.prjId.prjName}@@<ex:segDisplay id="${contractInfo.segid}"/>@@<ex:dicDisplay dictIdentify="CTRSORT" dictValue="${contractInfo.ctrSort}"/>@@<fmt:formatDate value="${contractInfo.signDate}" pattern="yyyy-MM-dd"/>@@<fmt:formatNumber pattern="#,##0.00" value="${contractInfo.ctrSum}"/>@@<fmt:formatNumber pattern="#,##0.00" value="${contractInfo.estimateSum}"/>' /> 
			</ec:column>
		</ec:row>
	</ec:table>
</body>
