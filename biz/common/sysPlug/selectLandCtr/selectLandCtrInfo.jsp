...
<script type="text/javascript">
	...
	$(document).ready(function(){
			...
			$("#plug_selectCtr").click(function(){
				...
				if(paramWindow==1){
					var url ='contentSelectAction.do?method=selectLandCtr&paramWindow=1&queryLandCtrType=${queryLandCtrType}&prj_id='+$("#prj_id").val();
					window.open(url,"土地合同信息","");
				}else{
					var url ='contentSelectAction.do?method=selectLandCtr&queryLandCtrType=${queryLandCtrType}&prj_id='+$("#prj_id").val();
					art.dialog({id:'commonDialog1',title:'合同信息', iframe:url, width:'700', height:'400',top:'0'}).close(function(){
					setParamLandCtrValue($("#plug_ctrId").val(),$("#plug_designCorpName").val(),$("#plug_ctrCode").val(),$("#plug_ctrName").val(),$("#plug_ctrSort").val(),$("#plug_payClause").val(),$("#plug_signDate").val(),$("#plug_ctrSum").val());
					});	
				}
			}); 
	});
