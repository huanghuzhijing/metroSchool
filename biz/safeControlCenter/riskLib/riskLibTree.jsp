...
<c:forEach items="${lines}" var="line">
  ...
  <c:forEach items="${segments}" var="segment">
    ...
    <c:forEach items="${segment.stations}" var="station">
        checkPointTree.add('ST_${station.station_id}','SS_${segment.segment_id}','${station.station_name}','riskLibAction.do?method=list&station_id=${station.station_id}&segment_id=${segment.segment_id}&station_state=${station.station_state}','','main','${ctx}/image/dtaq/station.png','${ctx}/image/dtaq/station.png');
    </c:forEach> // stations
  </c:forEach> // segments
