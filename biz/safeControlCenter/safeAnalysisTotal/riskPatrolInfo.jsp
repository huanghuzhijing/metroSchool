...
<body>
 		<c:if test="${liveVideo != null}">
	  			...
	  			<div class="main_div" align="center">
					...
					<div class="content_div">
						<c:if test="${liveVideo.oldOrNew != 1}">
							<a href="http://221.232.132.106:8089" target="_blank"><img id="ownerMap"  style="width:100%" alt="现场视频图片" src="${ctx}/liveVideoAction.do?method=viewMap&id=${liveVideo.id}&__skip__" border="0"></a>
						</c:if>
						<c:if test="${liveVideo.oldOrNew == 1}">
							<img id="ownerMap"  style="width:100%" alt="现场视频图片" src="http://113.57.131.39:80${liveVideo.attach_address}" border="0"></a>
						</c:if>
					</div>
				</div>
			</c:if>
			<c:if test="${liveVideo.memo != null}">
				<div class="main_div" align="center">
					<div class="title_div">现场视频摘要</div>
		...
		<c:if test="${riskPatrol != null}">
			<c:if test="${riskPatrol.attach_name != null}">
				<div align="right">
				 	<a href="${ctx}/safeAnalysisTotalAction.do?method=download&id=${riskPatrol.id}">[${riskPatrol.attach_name}]</a>
				</div>
			</c:if>
		</c:if>
	    <c:if test="${liveVideo == null}">
