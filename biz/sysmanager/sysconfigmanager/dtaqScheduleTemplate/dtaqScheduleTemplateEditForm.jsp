	...
	<script type="text/javascript">
	   ...
	   $(document).ready(function(){
			...
			$("#btn_save").click(function(){
				...
				if('${dtaqScheduleTemplate.station.station_type}'=='P'){
				properties[1]=["fileName3_","形象进度剖面图模板 "];
				}else{
...
<body>
	<html:form enctype="multipart/form-data" action="dtaqScheduleTemplateAction.do" method="POST">
		...
		<!--查询定位 开始-->
		<html:hidden property="template_id" value="${dtaqScheduleTemplate.template_id}"/>
		...
		<fieldset >
			...
			<table class="formTable" >
					...
					<tr>
						...
						<td >
							${dtaqScheduleTemplate.segment.segment_name}
						</td>
					</tr>
					...
					<tr>
						...
						<td>
						${dtaqScheduleTemplate.station.station_name}
						</td>
					...
					<tr>
						<td>
							...
							class="inputText" size="30" value="${dtaqScheduleTemplate.plane_name}" /></span>(dwg格式文件，大小2M以内!)
						<span style="float: inherit;margin-left:5px;"><input type="button" icon="icon_currWeek"
							onclick="javascript:showUploadDialogAndRefresh('dtaqScheduleTemplateAction.do?method=uploadFile&flag=1&mainTableName=${maintable}',900,700);"
						...
						<input type="button" icon="icon_delete" onclick="fileClear(this)"
							...
							<c:if test="${dtaqScheduleTemplate.plane_address!=null}">
								<a href="<%=request.getContextPath()%>/dtaqScheduleTemplateAction.do?method=download&type=plane&template_id=${dtaqScheduleTemplate.template_id}">${dtaqScheduleTemplate.plane_name}</a>
							</c:if>
<%--							<html:text property="fileName_" styleClass="inputText"	value="${compCompleteCheck.reportAnnex}" maxlength="100" size="30"/>--%>
						...
						<td style="display:none">
							<input type="text" id="fileAddr_" name="fileAddr_" class="inputText" readonly="readonly" size="30" value="${dtaqScheduleTemplate.plane_address}"/>
					...
					<tr>
						<td>
							...
							class="inputText" size="30" value="${dtaqScheduleTemplate.section_name}" /></span>(dwg格式文件，大小2M以内!)
						<span style="float: inherit;margin-left:5px;"><input type="button" icon="icon_currWeek"
							onclick="javascript:showUploadDialogAndRefresh('dtaqScheduleTemplateAction.do?method=uploadFile&flag=2&mainTableName=${maintable}',900,700);"
							style="width: 76px; height: 65px; cursor: pointer" value="文件上传"
							alt="进入文件上传页面" /></span>
						<%--							<input id="fileAddr_" class="inputText" type="file" onchange="fileSelect(this)" name="fileAddr_"/>--%>
						<input type="button" icon="icon_delete" onclick="fileClear2(this)"
							style="width: 76px; height: 65px; cursor: pointer" value="清空" />
							<c:if test="${dtaqScheduleTemplate.section_address!=null}">
								<a href="<%=request.getContextPath()%>/dtaqScheduleTemplateAction.do?method=download&type=section&template_id=${dtaqScheduleTemplate.template_id }">${dtaqScheduleTemplate.section_name}</a>
							</c:if>
<%--							<html:text property="fileName_" styleClass="inputText"	value="${compCompleteCheck.reportAnnex}" maxlength="100" size="30"/>--%>
						</td>
						<td style="display:none">
						...
						<td style="display:none">
							<input type="text" id="fileAddr2_" name="fileAddr2_" class="inputText" readonly="readonly" size="30" value="${dtaqScheduleTemplate.section_address}"/>
					 ...
					 <tr>
						<td>
							...
							class="inputText" size="30" value="${dtaqScheduleTemplate.schematic_name}" /></span>(dwg格式文件，大小2M以内!)
						<span style="float: inherit;margin-left:5px;"><input type="button" icon="icon_currWeek"
							onclick="javascript:showUploadDialogAndRefresh('dtaqScheduleTemplateAction.do?method=uploadFile&flag=3&mainTableName=${maintable}',900,700);"
							style="width: 76px; height: 65px; cursor: pointer" value="文件上传"
							alt="进入文件上传页面" /></span>
						<%--							<input id="fileAddr_" class="inputText" type="file" onchange="fileSelect(this)" name="fileAddr_"/>--%>
						<input type="button" icon="icon_delete" onclick="fileClear3(this)"
							...
							<c:if test="${dtaqScheduleTemplate.schematic_address!=null}">
								<a href="<%=request.getContextPath()%>/dtaqScheduleTemplateAction.do?method=download&type=schematic&template_id=${dtaqScheduleTemplate.template_id }">${dtaqScheduleTemplate.schematic_name}</a>
							</c:if>
<%--							<html:text property="fileName_" styleClass="inputText"	value="${compCompleteCheck.reportAnnex}" maxlength="100" size="30"/>--%>
						</td>
						<td style="display:none">
						...
						<td style="display:none">
							<input type="text" id="fileAddr3_" name="fileAddr3_" class="inputText" readonly="readonly" size="30" value="${dtaqScheduleTemplate.schematic_address}"/>
