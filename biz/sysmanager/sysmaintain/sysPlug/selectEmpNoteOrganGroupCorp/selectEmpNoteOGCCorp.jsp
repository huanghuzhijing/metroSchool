...
<!--		<input id="select_group" type="radio"  value="2" >按用户组选择	-->
		<input id="select_corp" type="radio"  value="3" checked="checked">按参建单位选择	
	</div>
	<fieldset>
	<legend>人员选择</legend>
			...
			<tr>
				<td valign="top" width="380px">
					<div style="width: 380px;text-align:left">
							...
							organizationTree.add('-1',-99,'组织树','${ctx}/employeeAction.do?method=selectEmpNoteOGCCorp&single=${param.single}');
							
							<c:forEach items="${organizationTypeTree}" var="organizationType">
								 organizationTree.add('${organizationType.data_short_name}','-1','${organizationType.data_name}','${ctx}/employeeAction.do?method=selectEmpNoteOGCCorp&single=${param.single}','','','','');
							</c:forEach>
							
							<c:forEach items="${organizationTree}" var="organization">
								organizationTree.add('${organization.f_id}','${organization.f_economictype}','${organization.f_name}(${organization.employeecount})','${ctx}/employeeAction.do?method=selectEmpNoteOGCCorp&qe_org=${organization.f_id}&single=${param.single}','','','','');
							...
							<tr>
								<td>
									<fieldset>
										...
										<div>
											<html:form
												action="employeeAction.do?method=selectEmpNoteOGCCorp&single=${param.single}">
