       ...
       <tr>
					...
					<script  type="text/javascript">
							...
							<c:forEach items="${employeestree}" var="organization">
									<c:if test = "${organization.parent_id == '-1'}">
										organizationTree.add('${organization.f_id}','${organization.parent_id}','${organization.f_name}','${ctx}/employeeAction.do?method=selectEmpsForSysOperator&static_fno=${organization.f_no}&static_orgid=${organization.f_id}&name=${organization.f_name}&qorganidentify=${param.qorganidentify}');
									</c:if>
									<c:if test="${organization.parent_id != '-1'}" >
										organizationTree.add('${organization.f_id}','${organization.parent_id}','${organization.f_name}(${organization.employeecount})','${ctx}/employeeAction.do?method=selectEmpsForSysOperator&static_fno=${organization.f_no}&static_orgid=${organization.f_id}&name=${organization.f_name}&qorganidentify=${param.qorganidentify}');
									</c:if>
							</c:forEach>
         ...
         <td valign="top">
	          <table class="queryTable" >
					...
					<tr>
						<td>
							<html:form action="employeeAction.do?method=selectEmpsForSysOperator" method="POST">
