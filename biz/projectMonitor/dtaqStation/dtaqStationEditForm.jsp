	 ...
	 //保存
		...
		//返回		
		 $("#btn_back").click(function(){ 
			document.forms[0].action='dtaqStationAction.do?method=index&ec_p=${param.ec_p}&ec_id=${param.station_id}&ec_crd=${param.ec_crd}&qsegmentId=${param.qsegmentId}&flagPage=3'; 
...
<table class="formTable">
	...
	<tr>
		...
		<td colspan="3"><html:file property="build_plane" size="30" styleClass="text" onchange="validatePictureSize(this)"/><font color="#50A2E6" size="2">(文件格式：gif,jpg,jpeg,png,bmp;大小：2280*2280以内)</font>
		 <c:if test="${dtaqStation.station_id ne null && dtaqStation.build_plane_address ne null}">
		      	<a href="${ctx}/dtaqStationAction.do?method=download&station_id=${dtaqStation.station_id}&download_type=build_plane">[${dtaqStation.build_plane_name}]</a>
		 </c:if>
	...
	<tr>
		...
		<td colspan="3"><html:file property="third_plane" size="30" styleClass="text" onchange="validatePictureSize(this)"/><font color="#50A2E6" size="2">(文件格式：gif,jpg,jpeg,png,bmp;大小：2280*2280以内)</font>
		 <c:if test="${dtaqStation.station_id ne null && dtaqStation.third_plane_address ne null}">
		      	<a href="${ctx}/dtaqStationAction.do?method=download&station_id=${dtaqStation.station_id}&download_type=third_plane">[${dtaqStation.third_plane_name}]</a>
		      </c:if>
	...
	<tr>
		...
		<ex:tab id="dockjTab" width="100%">
			...
			<ex:tabItem name="lab1" url="dtaqStationAction.do?method=edit&tabFlag=1&qsegmentId=${param.qsegmentId}&station_id=${param.station_id}" selected="${tabstate1}" label="老系统描点" >
				<c:if test="${param.tabFlag == '1'}">
					<%@ include file="/biz/projectMonitor/dtaqStation/drawPicPiontOld.jsp" %>
				</c:if>
			   </ex:tabItem>
		   <ex:tabItem name="lab2"  url="dtaqStationAction.do?method=edit&tabFlag=2&qsegmentId=${param.qsegmentId}&station_id=${param.station_id}" label="新系统描点" selected="${tabstate2}">
