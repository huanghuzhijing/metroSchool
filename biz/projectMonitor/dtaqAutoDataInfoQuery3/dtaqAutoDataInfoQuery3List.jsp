	...
	//	}
		...
		function viewInfo(id) {
			...
			var params = "&qlineId="+qlineId+"&qsegmentId="+qsegmentId+"&qstationId="+qstationId+"&qtypeId="+qtypeId+"&qstartDate="+qstartDate+"&qendDate="+qendDate+"&qorganType="+qorganType+"&dtaqDataInfoList_page=${dtaqDataInfoList_page}";
		  	location.href="${ctx}/dtaqAutoDataInfoQuery3Action.do?method=view&thirdList=${param.thirdList}&info_id="+id+params;
		  	
		}
		//导出
		function exportExcel(id) {
		  location.href="${ctx}/dtaqAutoDataInfoQuery3Action.do?method=exportExcel&info_id="+id;
		}
	</script>
</head>
<body >
	<table class="queryTable" >
		...
		<tr>
			<td>
				<html:form action="dtaqAutoDataInfoQuery3Action.do?method=list&ec_p=${param.ec_p}&ec_id=${param.id}&ec_crd=${param.ec_crd}&qlineId=${qlineId}&qsegmentId=${qsegmentId}&qstationId=${qstationId}&thirdList=${param.thirdList}">
					<table align="left">
						<tr>
...
<ec:table items="dtaqAutoDataInfos" var="dtaqAutoDataInfo" action="dtaqAutoDataInfoQuery3Action.do?method=list"
	...
	retrieveRowsCallback="limit" filterRowsCallback="limit" sortRowsCallback="limit">
  	<ec:row highlightRow="true" >
		...
		<ec:column property="write_date" title="填报日期" width="10%">
			<fmt:formatDate value="${dtaqAutoDataInfo.write_date}" pattern="yyyy-MM-dd HH:mm:ss"/>
		</ec:column>
		<ec:column property="monitoring_date" title="监测日期" width="10%">
			<fmt:formatDate value="${dtaqAutoDataInfo.monitoring_date}" pattern="yyyy-MM-dd HH:mm:ss"/>
		</ec:column>
		<ec:column property="write_user" title="填报人" width="10%">
			<ex:employeeName id="${dtaqAutoDataInfo.write_user}" />
		</ec:column>
		<ec:column property="null" title="操作" width="12%">
			<div align="center">
				<input type="button" class="button1" value="查看明细" onclick="viewInfo('${dtaqAutoDataInfo.info_id}')">
		    	<input type="button" class="button1"value="导出Excel" onclick="exportExcel('${dtaqAutoDataInfo.info_id}')">
			</div>
		</ec:column>
