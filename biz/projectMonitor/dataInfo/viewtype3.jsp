					...
					<table border="0" cellspacing="0" width="100%" cellpadding="0"
						...
						<tbody>
							<c:forEach items="${datas}" var="data">
								<tr title="${data.warn_memo}"><td  onclick="showPointPosition(this)">${data.point_code}</td><td>${data.curr_value}</td><td>${data.change_rate}</td><td>${data.total_change}</td><td><span class="state${data.warn_state}" style="width:50px;display:block;height:18px;"></div></td><td align="center"><input type="button"value="时序曲线" class="button1"onclick="showComparePointTimes3(${data.point_id.point_id})">&nbsp;</td></tr>
							</c:forEach>
						</tbody>
					</table>
				</div>
