...
<script type="text/javascript">
	...
	//				alert("type_id="+jQuery("[name='type_id']").val()+"&station_id=${param.station_id}&organ_type=${param.organ_type}&picturePart=Y");
						$.ajax({
							type: "POST",
							url: "${ctx}/checkpointPictureAction.do?method=queryPart",
							data: "type_id="+jQuery("[name='type_id']").val()+"&station_id=${param.station_id}&organ_type=T&picturePart=Y",
...
//		   alert(selRequest);
		...
		function exportPoints(){
			var url="checkpointManageAction.do?method=exportExcelPoints&organ_type=T&isthird=Y&line_id=${line_id}&segment_id=${segment_id}&station_id=${param.station_id}&type_id="+jQuery("[name='type_id']").val()+"&part_id="+jQuery("[name='part_id']").val()+"&point_code="+jQuery("[name='point_code']").val();
			location.href=url;
			}
...
<ex:tab id="dataInfo">
	<ex:tabItem name="work" label="施工方" url="checkpointManageAction.do?method=queryPointList&organ_type=O&isthird=N&line_id=${param.line_id}&segment_id=${param.segment_id}&station_id=${param.station_id}" disabled="false" />
	<ex:tabItem name="third" label="第三方" selected="true">
		 <fieldset>
		  ...
		  <html:hidden property="isthird" value="Y"/>
