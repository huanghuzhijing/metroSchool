...
<td>
<div class="saveDiv">
<input type="button" id="btnStart_point" class="button1" value="开始绘点"> 
<input type="button" id="btnDelete"   class="button1" value="删除绘点"> 
<input type="button" id="btnSave" class="button1" value="保存绘点"> 
...
$(function(){
	...
	var _pointx='${point.pointx}';
	if(_pointx!=''&&_pointx!=null){		
	     pointCoordsArr = [${point.pointx},${point.pointy}]
	}
	...
	_thisPage.find('#btnSave').click(function(){
	$.ajax({
	type:"post",
	url:"dtaqDataQueryAction.do?method=savePoint&qstationId=${point.partId.station_id.station_id}&qtype_id=${point.type_id}&point_id="+$("[name='point_id']").val()+"&pointx="+$("[name='pointx']").val()+"&pointy="+$("[name='pointy']").val(),
	success:function(){
	alert("保存成功!");
	}
	})
