   ...
   var oper_id=""; 
	 $(document).ready(function(){
       ...
       $("#btn_add").click(function(){
       		location.href='dtaqCheckpointAction.do?method=create&organ_type=${organ_type}&qlineId=${qlineId}&qsectionId=${qsectionId}&station_id=${station_id}&type_id=${type_id}&part_id=${part_id}&station_state=${station_state}';
       });
       //
		...
		$("#btn_delete").click(function(){ 
			...
			if(ret){
				var url='dtaqCheckpointAction.do?method=delete&organ_type=${param.organ_type}&point_id='+oper_id+'&ec_p='+$("input[name='ec_p']").val();
				commonUrl(url);
			}
		});
		//
...
所属部位：<span class="redSpan"><ex:partId id="${part_id}"/></span>
	...
	<c:if test="${station_state=='1'}">
		...
		<ec:table items="dtaqCheckpoints" var="dtaqCheckpoint"
			action="dtaqCheckpointAction.do" title=""  locale="zh_CN"
