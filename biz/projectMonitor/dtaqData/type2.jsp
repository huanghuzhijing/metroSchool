...
function setEmptyField(idx){
    ...
    jQuery("#curr_value_"+idx).value = "";
    jQuery("#v_curr_value_"+idx).html("");
...
function valueChanged(idx) {
  var valueEl = jQuery("#curr_change_"+idx);
  var curr_change = valueEl.val().trim();
  var hiddendata = jQuery("#hiddata_"+idx).val() ;
  var hdata = hiddendata.split("@");
  var operate_mode = jQuery("[name='operate_mode']").val();
  if(curr_change.length==0) {
    setEmptyField(idx);
  } else if(curr_change == " ") {
    setEmptyField(idx);
  }else if(isNaN(curr_change)){
  	setEmptyField(idx);
    alert("输入值不是合法数值。");
    valueEl.value = "";
    valueEl.focus();
   ...
   var warn_state = 1; // 预警状态
    ...
    var d_curr_change = parseFloat(curr_change);
	...
	var change_rate = (d_curr_change/days).toFixed(2);
	var total_change = (d_curr_change + d_prev_total_value).toFixed(2);
	var curr_value = (d_init_value + total_change/1000).toFixed(5);
	jQuery("#total_change_"+idx).val(total_change);
	jQuery("#v_total_change_"+idx).html(total_change);
	jQuery("#curr_value_"+idx).val(curr_value);
	jQuery("#v_curr_value_"+idx).html(curr_value);
	...
	if(warn_state == 4){
		if(abs_change_error == -3){
			memo = "累计位移(收敛)值小于负绝对警戒值";
		}else if(abs_change_error == 3){
			memo = "累计位移(收敛)值大于等于正绝对警戒值";
		}
	}
	//黄色报警
		...
		if(rel_change_error == 3){
			if(abs_change_error == -2){
				memo = "累计位移(收敛)值大于等于负绝对警戒值小于负绝对临界值,且位移(收敛)速率大于等于相对警戒值";
			}else if(abs_change_error == 1){
				memo = "累计位移(收敛)值大于等于负绝对临界值小于正绝对临界值,且位移(收敛)速率大于等于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "累计位移(收敛)值大于等于正绝对临界值小于正绝对警戒值,且位移(收敛)速率大于等于相对警戒值";
			}
		}else if(rel_change_error == 2){
			if(abs_change_error == -2){
				memo = "累计位移(收敛)值大于等于负绝对警戒值小于负绝对临界值,且位移(收敛)速率大于等于相对临界值小于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "累计位移(收敛)值大于等于正绝对临界值小于正绝对警戒值,且位移(收敛)速率大于等于相对临界值小于相对警戒值";
			}
		}
	}
	//蓝色报警
		...
		if(rel_change_error == 2){
			if(abs_change_error == 1){
				memo =  "累计位移(收敛)值大于等于负绝对临界值小于正绝对临界值,且位移(收敛)速率大于等于相对临界值小于相对警戒值";
			}
		}
		else if(rel_change_error == 1){
			if(abs_change_error == -2){
				memo =  "累计位移(收敛)值大于等于负绝对警戒值小于负绝对临界值,且位移(收敛)速率小于相对临界值";
			}else if(abs_change_error == 2){
				memo =  "累计位移(收敛)值大于等于正绝对临界值小于正绝对警戒值,且位移(收敛)速率小于相对临界值";
			}
		}
	}
	jQuery("#warn_memo_"+idx).value = memo;
...
<div class="tableRegion">
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td>
				<div class="grid">
					<table border="0" cellspacing="0" cellpadding="0" style="border-collapse: collapse;" id="contractInfoList" width="100%">
						<thead>
							<tr>
								<th width="10%">测点编码</th>
								<th width="10%">监测部位</th>
						...
						<tbody>
							...
							<c:forEach items="${points}" var="point">
								...
								<td align="center">${point.point_code}
									...
									<input type="hidden" name="curr_value" id="curr_value_${idx}" value="${point.curr_value}">
									<input type="hidden" name="load_curr_change" id="load_curr_change_${idx}" value="${point.curr_change}">
								...
								<td align="center" id="v_curr_value_${idx}"></td>
								<td align="center" id="v_prev_value_${idx}">${point.curr_value}</td>
								<td align="center"><input type="text" class="lineinput" style="ime: disabled; width: 100%" name="curr_change" id="curr_change_${idx}" value="" onchange="valueChanged(${idx})"></td>
								...
								<c:if test="${operate_mode == 'edit'}">
									<td align="center">
										<input type="button" value="时序曲线" class="button1"onclick="showPointTimes(${point.point_id})">
									</td>
								</c:if>
							</tr>
