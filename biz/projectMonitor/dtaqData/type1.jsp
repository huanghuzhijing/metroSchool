	...
	//黄色报警
		...
		if(rel_change_error == 3){
			if(abs_change_error == -2){
				memo = "累计沉降值大于等于负绝对警戒值小于负绝对临界值,且沉降速率大于等于相对警戒值";
			}else if(abs_change_error == 1){
				memo = "累计沉降值大于等于负绝对临界值小于正绝对临界值,且沉降速率大于等于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "累计沉降值大于等于正绝对临界值小于正绝对警戒值,且沉降速率大于等于相对警戒值";
			}
		}else if(rel_change_error == 2){
			if(abs_change_error == -2){
				memo = "累计沉降值大于等于负绝对警戒值小于负绝对临界值,且沉降速率大于等于相对临界值小于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "累计沉降值大于等于正绝对临界值小于正绝对警戒值,且沉降速率大于等于相对临界值小于相对警戒值";
			}
		}
	}
	//蓝色报警
		...
		if(rel_change_error == 2){
			if(abs_change_error == 1){
				memo =  "累计沉降值大于等于负绝对临界值小于正绝对临界值,且沉降速率大于等于相对临界值小于相对警戒值";
			}
		}
		else if(rel_change_error == 1){
			if(abs_change_error == -2){
				memo =  "累计沉降值大于等于负绝对警戒值小于负绝对临界值,且沉降速率小于相对临界值";
			}else if(abs_change_error == 2){
				memo =  "累计沉降值大于等于正绝对临界值小于正绝对警戒值,且沉降速率小于相对临界值";
			}
		}
	}
...
<div class="tableRegion">
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			<td>
				<div class="grid">
					<table border="0" cellspacing="0" width="100%" cellpadding="0" style="border-collapse: collapse;" id="contractInfoList">
						<thead>
							<tr>
								<th width="10%">测点编码</th>
								<th width="10%">监测部位</th>
						...
						<tbody>
							...
							<c:forEach items="${points}" var="point">
								<tr id="tr_${idx}">
									...
									<c:if test="${operate_mode == 'edit'}">
										<td align="center"><input type="button" value="时序曲线" class="button1" onclick="showPointTimes(${point.point_id})"></td>
									</c:if>
								</tr>
