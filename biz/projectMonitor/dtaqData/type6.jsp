...
function setEmptyField(idx){
    ...
    jQuery("#curr_change_"+idx).html("");
    jQuery("#v_curr_change_"+idx).val("");
...
function valueChanged(idx) {
  ...
  if(curr_value.length==0) {
    ...
    var d_pre_value = parseFloat(jQuery("#prev_value_"+idx).val());
	// 累计沉降
	var curr_change = (d_curr_value - d_pre_value).toFixed(2);;
	var total_change = (d_curr_value - d_init_value).toFixed(2);
	//计算速率
   var days = 1;
	...
	var change_rate = (curr_change/days).toFixed(2);
    jQuery("#total_change_"+idx).val(total_change);
 	jQuery("#curr_change_"+idx).val(curr_change);
    jQuery("#v_curr_change_"+idx).html(curr_change);
    jQuery("#change_rate_"+idx).val(change_rate);
    jQuery("#v_change_rate_"+idx).html(change_rate);
	change_rate = Math.abs(change_rate);
	//判断累计的绝对值状态
	
	if (d_curr_value >= abs_warning_value) {
		abs_change_error = 3;
	}else if (d_curr_value >= abs_critical_value) {
		abs_change_error = 2;
	}else if(d_curr_value >= negative_abs_critical_value){
		abs_change_error = 1;
	}else if(d_curr_value >= negative_abs_warning_value){
		abs_change_error = -2;
	}else if(d_curr_value < negative_abs_warning_value){
		abs_change_error = -3;
	}
	...
	//黄色报警
		...
		if(rel_change_error == 3){
			if(abs_change_error == -2){
				memo = "本次温度值大于等于负绝对警戒值小于负绝对临界值,且变化速率大于等于相对警戒值";
			}else if(abs_change_error == 1){
				memo = "本次温度值大于等于负绝对临界值小于正绝对临界值,且变化速率大于等于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "本次温度值大于等于正绝对临界值小于正绝对警戒值,且变化速率大于等于相对警戒值";
			}
		}else if(rel_change_error == 2){
			if(abs_change_error == -2){
				memo = "本次温度值大于等于负绝对警戒值小于负绝对临界值,且变化速率大于等于相对临界值小于相对警戒值";
			}else if(abs_change_error == 2){
				memo = "本次温度值大于等于正绝对临界值小于正绝对警戒值,且变化速率大于等于相对临界值小于相对警戒值";
			}
		}
	}
	//蓝色报警
		...
		if(rel_change_error == 2){
			if(abs_change_error == 1){
				memo =  "本次温度值大于等于负绝对临界值小于正绝对临界值,且变化速率大于等于相对临界值小于相对警戒值";
			}
		}
		else if(rel_change_error == 1){
			if(abs_change_error == -2){
				memo =  "本次温度值大于等于负绝对警戒值小于负绝对临界值,且变化速率小于相对临界值";
			}else if(abs_change_error == 2){
				memo =  "本次温度值大于等于正绝对临界值小于正绝对警戒值,且变化速率小于相对临界值";
			}
		}
	}
					...
					<table border="0" cellspacing="0" cellpadding="0"
						...
						<tbody>
							...
							<c:forEach items="${points}" var="point">
								<tr id="tr_${idx}">
								<td align="center">${point.point_code}<input type="hidden" name="point_id" id="point_id_${idx}" value="${point.point_id}">
								...
								<input type="hidden" name="prev_value" id="prev_value_${idx}" value="${point.curr_value}">
								<input type="hidden" name="curr_change" id="curr_change_${idx}" value="${point.curr_change}">
								...
								<input type="hidden" name="hiddata" id="hiddata_${idx}" value="${point.point_id}@${point.point_code}@${point.partId.part_id}@${point.init_date}@${point.init_value}@${point.write_date}@${point.total_change}@${point.curr_value}@${point.abs_critical_value}@${point.abs_warning_value}@${point.negative_abs_critical_value}@${point.negative_abs_warning_value}@${point.rel_critical_value}@${point.rel_warning_value}@${point.prev_date}@${point.t_pre_curr_value}"></td>
								<td align="center">${point.partId.part_name}</td>
								<td align="center">${point.init_value}</td>
								<td align="center"><input type="text" class="lineinput" name="curr_value" id="curr_value_${idx}" value="" style="ime: disabled; width: 100%"onchange="valueChanged(${idx})"></td>
								<td align="center" id="v_prev_value_${idx}">${point.curr_value}</td>
								<td align="center" id="v_curr_change_${idx}"></td>
								<td align="center" id="v_change_rate_${idx}"></td>
								<td align="center">${point.abs_warning_value}</td>
								<td align="center"><div id="v_warn_state_${idx}"></div></td>
								<c:if test="${operate_mode == 'edit'}"><td align="center"><input type="button" value="${texts['dtaq.data.time_curve']}" class="button1" onclick="showPointTimes(${point.point_id})"></td></c:if></tr>
