...
<body>
	<html:form enctype="multipart/form-data" action="prjCorpUserJoinAction.do?method=saveEmpForConstr" method="POST">
		<input id="empIds" name="empIds"  type="hidden">
		<input name="prjid" value="${param.prjid }" type="hidden">
		<input name="segmentid" value="${param.segmentid }" type="hidden">
		<input name="ctrid" value="${param.ctrid }" type="hidden">
		<input name="corpid" value="${param.corpid }" type="hidden">
	</html:form>
	<div class="promptDiv">
		所属单位名称：<span class="redSpan">${corpname}</span>	
	</div>
	<ec:table items="employees1" var="employee" 
		action="prjCorpUserJoinAction.do?method=selectEmpForConstr" title=""  locale="zh_CN"
