...
<script type="text/javascript">
	...
	checkCmd();
		...
		$("#btn_back").click(function(){ 
			var url='projectInfoAction.do?method=listRight&staticPrjStatus=${param.staticPrjStatus}&ec_p=${param.parent_ec_p}&ec_id=${projectInfo.id}&ec_auditStatus=${projectInfo.auditStatus}&staticPrjSort='+$("[name='staticPrjSort']").val()+'&staticFootNode='+$("[name='staticFootNode']").val()+'&flag='+$("[name='flag']").val();
			commonUrl(url);
		}); 
		
		$(".btn_addSingle").click(function(){//工点管理
			var url='prjSingleBuildingAction.do?method=list&staticPrjId=${param.staticPrjId}&staticSegId='+$(this).attr("paramId");
			commonUrl(url);
		}); 
		//
		$("#btn_add").click(function(){ 
			var url='prjSegmentInfoAction.do?method=create&staticPrjStatus=${param.staticPrjStatus}&staticPrjId=${param.staticPrjId}'+'&ec_p='+$("input[name='ec_p']").val()+'&parent_ec_p=${param.parent_ec_p}'+'&staticPrjSort=${param.staticPrjSort}&staticFootNode=${param.staticFootNode}&flag=${param.flag}&flagPage=1';
		...
		$(".btn_edit").click(function(){
			var url='prjSegmentInfoAction.do?method=edit&staticPrjStatus=${param.staticPrjStatus}&staticPrjId=${param.staticPrjId}&id='+$(this).attr("paramId")+'&ec_p='+$("input[name='ec_p']").val()+'&parent_ec_p=${param.parent_ec_p}'+'&staticPrjSort=${param.staticPrjSort}&staticFootNode=${param.staticFootNode}&flag=${param.flag}&flagPage=1';
			
			 art.dialog({id:'commonDialog1',title:'标段管理-编辑', iframe:url, width:'800', height:'400',top:'0'});
		//	commonUrl(url);
		}); 
		//
		$(".btn_delete").click(function(){ 
			...
			if(ret){
				var url='prjSegmentInfoAction.do?method=delete&staticPrjStatus=${param.staticPrjStatus}&staticPrjId=${param.staticPrjId}&id='+$(this).attr("paramId")+'&ec_p='+$("input[name='ec_p']").val()+'&parent_ec_p=${param.parent_ec_p}'+'&staticPrjSort=${param.staticPrjSort}&staticFootNode=${param.staticFootNode}&flag=${param.flag}&flagPage=2';
				commonUrl(url);
			}
		});
		//
		$(".btn_submit").click(function(){ 
			...
			if(ret){
				var url='prjSegmentInfoAction.do?method=issue&staticPrjStatus=${param.staticPrjStatus}&staticPrjId=${param.staticPrjId}&id='+$(this).attr("paramId")+'&ec_p='+$("input[name='ec_p']").val()+'&parent_ec_p=${param.parent_ec_p}'+'&staticPrjSort=${param.staticPrjSort}&staticFootNode=${param.staticFootNode}&flag=${param.flag}&flagPage=2';
				commonUrl(url);
			}
		});
		//
	...
	function pageRefresh(){
		var url='prjSegmentInfoAction.do?method=listRight&staticPrjStatus=${param.staticPrjStatus}&staticPrjId=${param.staticPrjId}'+'&ec_p='+$("input[name='ec_p']").val()+'&parent_ec_p=${param.parent_ec_p}'+'&staticPrjSort=${param.staticPrjSort}&staticFootNode=${param.staticFootNode}&flag=${param.flag}';
		commonUrl(url);
	}
</script>
</head>
	...
	>
		...
		>
			...
			<ec:column property="null" title="${texts['label.operate']}" >
				...
				<input  value="标段选择"  id="btn_addSingle"  type="hidden" icon="icon_add" class="btn_addSingle" paramId="${prjSegmentInfo.id}"/>
