...
<c:forEach items="${lines}" var="line">
  ...
  <c:forEach items="${segments}" var="segment">
   ...
   <c:forEach items="${segment.stations}" var="station">
	     checkPointTree.add('ST_${station.station_id}','SS_${segment.segment_id}','${station.station_name}','dtaqDataQueryAction.do?method=list&qstationId=${station.station_id}',null,null,'${ctx}/image/dtaq/station.png','${ctx}/image/dtaq/station.png');
       </c:forEach>
  </c:forEach> // segments
